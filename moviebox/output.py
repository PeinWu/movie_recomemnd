...
def prettyPrint(movieID,title,category,plot,color,showPlots):
    ...
    print(' Categories: '+prettyCategory+'\n',color)
    if (showPlots):
        print(colored(' Plot:\n'+plot+'\n',color))
...
def validateInput(movieID,recommendationsNumber):
    if not (isinstance(movieID,int) and movieID>=0 and movieID<=4999):
        printRed('Invalid value for movieID:'+str(movieID)+'"')
        printRed('Input is not a valid Natural number between [0,4999]')
        sys.exit(1)

    if not (isinstance(recommendationsNumber,int) and recommendationsNumber >=1 and recommendationsNumber<=30):
        printRed('Invalid value for recommendations number: '+str(recommendationsNumber)+'"')
